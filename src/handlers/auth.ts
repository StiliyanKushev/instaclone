...
export function changePassword(state: IChangePasswordState, email: string) {
    ...
    return post<IChangePasswordPost, IGenericResponse>({ ...state, email }, '/auth/password-change');
}
...
export function editProfile(state: IEditProfileState, email: string) {
    ...
    return post<IEditProfilePost, IEditProfileResponse>({ ...state, authEmail: email }, '/auth/edit-profile');
}